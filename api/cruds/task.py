...
#   - FastAPI에서 async def 함수와 함께 사용하며, commit/refresh 같은 작업 앞에는 await를 붙어야 한다.
...
# --------------------------------------------------------------------------
# [ 함수: create_task ]
# 사용자가 보낸 "할 일" 정보를 받아서 질제 DB에 저장하는 함수
# --------------------------------------------------------------------------
...
# * 반환값: 없음 (삭제만 수행하고 결과는 따로 잔환하지 않음)
...
async def get_tasks_with_done(db: AsyncSession) -> list[tuple[int, str, bool]]:
    result: Result = await db.execute(
        select(
            ...
            #   -> 쉽게 말해, '모든 할 일'을 다 불러오고, 그 중에서 완ㄹ된 것도표시하는 방식
        ).outerjoin(
            task_model.Done
        )  # ? outerjoin: 할 일 이 완료됐든 안 됐든 모두 가져오기
    )
