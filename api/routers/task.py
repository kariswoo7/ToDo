...
async def list_tasks():
    return [task_schema.Task(id=1, title="첫 번쨰 ToDo 작업", done=False)]
...
async def create_task(
    ...
    #   - 테스트 시에는 get_db 함수를 오버라이드해서 가짜 DB나 데스트용 DB를 넣을 수 잇음
    #
    # * 이 구조는 DB 작업(비즈니스 로직)은 crud.py에 따로 만들고,
    #   이 함수는 오쳥 받고 응답하는 역할만 담당하도록 나눠서 구성함
    #   -> 코드가 더 깔끔하고 관리하기 쉬워짐
...
# - 클라이언트가 수정할 내용을 JSON으로 보내면 title을 바꿔주는 역활
# -----------------------------------------------------------------
...
# -----------------------------------------------------------------
# [4] 할 일 삭제 기능 (DELETE 요청청)
# - /tasks/번호 형식으로 요청이 오면 해당 번호의 할 일을 삭제함
# - 이 함수는 아직 DB가 없기 떄문에 동작은 하지 않지만 구조만 정의함
# -----------------------------------------------------------------
